//
// Created by Mr.zhou on 2024-08-09.
//
...
W25Q_STATE W25Q_Init(void) {
    ...
    /* If power-default 4-byte     mode disabled */
    if (!w25q_status.ADP) {
        uint8_t buf_reg = 0;
    ...
    /* If current 4-byte
     mode disabled */
    if (!w25q_status.ADS) {state = W25Q_Enter4ByteMode(1);
...
//    W25Q_STATE state;
    ...
    while((W25Q_ReadStatusReg(NULL,1)&0x01)==0x01);   // 等待BUSY位清空
//    uint8_t sr = 0;
//
